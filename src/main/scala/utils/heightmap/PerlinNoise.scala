...
object PerlinNoise{
  ...
  private def fastfloor(x: Float) = if (x > 0) x.toInt
  else x.toInt - 1

  private def dot(g: Array[Int], x: Float, y: Float, z: Float) = g(0) * x + g(1) * y + g(2) * z

  private def mix(a: Float, b: Float, t: Float) = (1 - t) * a + t * b

  private def fade(t: Float) = t * t * t * (t * (t * 6 - 15) + 10)
  ...
  def noise(xin: Float, yin: Float, zin: Float): Float = { // Find unit grid cell containing point
    ...
    val gi000 = perm(X + perm(Y + perm(Z))) % 12
    val gi001 = perm(X + perm(Y + perm(Z + 1))) % 12
    val gi010 = perm(X + perm(Y + 1 + perm(Z))) % 12
    val gi011 = perm(X + perm(Y + 1 + perm(Z + 1))) % 12
    val gi100 = perm(X + 1 + perm(Y + perm(Z))) % 12
    val gi101 = perm(X + 1 + perm(Y + perm(Z + 1))) % 12
    val gi110 = perm(X + 1 + perm(Y + 1 + perm(Z))) % 12
    val gi111 = perm(X + 1 + perm(Y + 1 + perm(Z + 1))) % 12
