...
case class Branch[V](bounds: AABox, partitions: IndexedSeq[Octree[V]], kvPairs: Seq[(AABox, V)])() extends Octree[V] {
  ...
  override def remove(value: V): Branch[V] = Branch(bounds, partitions.map(_.remove(value)),kvPairs.filterNot(kv2 => kv2._2.equals(value)))
  ...
  override def remove(area: AABox, v: V): Octree[V] = {
    ...
    if (pId == -1) { //v not here
      ...
      Branch(bounds, partitions, kvPairs.filterNot( kv => kv._2.equals(v) && kv._1.intersects(area)))
    } else {
